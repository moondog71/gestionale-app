...
interface Attachment { id: string; filename: string; driveUrl: string; mimeType: string; sizeBytes: number }
...
function calcTotal(parts: Part[], hours: string, rate: string) {
  return parts.reduce((s,p) => s + p.qty * p.unitPrice, 0) + parseFloat(hours||'0') * parseFloat(rate||'0')
}
...
export default function InterventiPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(await apiFetch<any[]>(`/interventi?${params}`))
  ...
  const openCreate = () => {
    setSelected(null); setForm({ ...EMPTY_FORM, date: new Date().toISOString().slice(0,10) })
    setAttachments([]); setModalOpen(true)
  }
  const openEdit = (item: any) => {
    setSelected(item)
    setForm({
      clientId: item.clientId||'', date: item.date?.slice(0,10)||'',
      addressOverride: item.addressOverride||'', applianceType: item.applianceType||'Lavatrice',
      brand: item.brand||'', model: item.model||'', serial: item.serial||'',
      workType: item.workType||'riparazione', description: item.description||'',
  ...
  const addPart = () => setForm((f: any) => ({ ...f, parts: [...f.parts, {description:'',qty:1,unitPrice:0}] }))
  const removePart = (i: number) => setForm((f: any) => ({ ...f, parts: f.parts.filter((_: any, idx: number) => idx !== i) }))
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = { ...form,
        ...
        totalAmount: total, stampDuty: total > 77.47,
      }
      if (selected) await apiFetch(`/interventi/${selected.id}`, { method:'PUT', body:JSON.stringify(payload) })
      else await apiFetch('/interventi', { method:'POST', body:JSON.stringify(payload) })
  ...
  const handleDelete = async () => {
    try { await apiFetch(`/interventi/${selected.id}`, { method:'DELETE' }); setDeleteOpen(false); load() }
  ...
  const handleUploadImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('logo', file) // campo generico — il backend usa req.file()
      const token = getToken()
      const res = await fetch(`/api/interventi/${selected.id}/attachments`, {
  ...
  const handleDeleteAttachment = async (attId: string) => {
    ...
    } catch { setError('Errore eliminazione allegato') }
  }
    ...
    <Layout>
      ...
      <Box sx={{ flex:1, overflow:'auto', p:2.5 }}>
        <Box sx={{ display:'flex', gap:2, mb:2 }}>
          <TextField placeholder="Cerca..." size="small" value={search} onChange={e => setSearch(e.target.value)} sx={{ flex:1 }}
        ...
        : items.length === 0 ? (
          ...
          <Stack spacing={1}>
            {items.map(item => {
              ...
              const hasImgs = (item.attachments?.length||0) > 0
              return (
                <Box key={item.id} sx={{
                  ...
                  <Box sx={{ flex:1, minWidth:0 }}>
                    <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>
                      ...
                      {hasImgs && (
                        <Tooltip title={`${item.attachments.length} immagini`}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {item.applianceType}{item.brand?` · ${item.brand}`:''}{item.model?` ${item.model}`:''} · {item.workType}
                    </Typography>
                  </Box>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selected ? `Modifica ${selected.number}` : 'Nuovo rapporto di intervento'}</DialogTitle>
        <DialogContent>
          ...
          <TextField label="Indirizzo intervento (se diverso da anagrafica)" fullWidth size="small" sx={{ mb:2 }}
            value={form.addressOverride} onChange={e => setForm((f: any) => ({ ...f, addressOverride: e.target.value }))} />
          ...
          <Box sx={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:2, mb:2 }}>
            <FormControl size="small"><InputLabel>Tipo</InputLabel>
              <Select value={form.applianceType} label="Tipo" onChange={e => setForm((f: any) => ({ ...f, applianceType: e.target.value }))}>
                {APPLIANCE_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
              </Select>
            </FormControl>
            <TextField label="Marca" size="small" value={form.brand} onChange={e => setForm((f: any) => ({ ...f, brand: e.target.value }))} />
            <TextField label="Modello" size="small" value={form.model} onChange={e => setForm((f: any) => ({ ...f, model: e.target.value }))} />
          </Box>
          <Box sx={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:2, mb:2 }}>
            <TextField label="Matricola / N. serie" size="small" value={form.serial} onChange={e => setForm((f: any) => ({ ...f, serial: e.target.value }))} />
            <FormControl size="small"><InputLabel>Tipo lavoro</InputLabel>
              <Select value={form.workType} label="Tipo lavoro" onChange={e => setForm((f: any) => ({ ...f, workType: e.target.value }))}>
                {WORK_TYPES.map(t => <MenuItem key={t} value={t} sx={{ textTransform:'capitalize' }}>{t}</MenuItem>)}
              </Select>
            </FormControl>
          </Box>

          <TextField label="Descrizione lavoro svolto" fullWidth multiline rows={3} size="small" sx={{ mb:2 }}
            value={form.description} onChange={e => setForm((f: any) => ({ ...f, description: e.target.value }))} />

          <Divider sx={{ my:1.5 }}><Typography variant="caption" color="text.secondary">RICAMBI E MANODOPERA</Typography></Divider>
          {form.parts.length > 0 && (
              ...
              <Table size="small">
                ...
                <TableBody>
                  {form.parts.map((p: Part, i: number) => (
                    <TableRow key={i}>
                      <TableCell><TextField variant="standard" value={p.description} size="small" fullWidth
                        onChange={e => updatePart(i,'description',e.target.value)} placeholder="Descrizione" /></TableCell>
                      <TableCell align="center"><TextField variant="standard" value={p.qty} type="number" size="small"
                        inputProps={{ min:1, style:{textAlign:'center'} }} sx={{ width:50 }}
                        onChange={e => updatePart(i,'qty',e.target.value)} /></TableCell>
                      <TableCell align="right"><TextField variant="standard" value={p.unitPrice} type="number" size="small"
                        inputProps={{ min:0, step:0.01, style:{textAlign:'right'} }} sx={{ width:90 }}
                        onChange={e => updatePart(i,'unitPrice',e.target.value)} /></TableCell>
                      <TableCell align="right"><Typography variant="body2" fontWeight={500}>{formatEur(p.qty*p.unitPrice)}</Typography></TableCell>
                      <TableCell><IconButton size="small" color="error" onClick={() => removePart(i)}><RemoveCircleOutlineIcon fontSize="small"/></IconButton></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <Button startIcon={<AddCircleOutlineIcon/>} size="small" onClick={addPart} sx={{ mb:2 }}>Aggiungi ricambio</Button>

          <Box sx={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:2, mb:2 }}>
            <TextField label="Ore lavoro" size="small" type="number" value={form.laborHours}
              inputProps={{ min:0, step:0.5 }} onChange={e => setForm((f: any) => ({ ...f, laborHours: e.target.value }))} />
            <TextField label="Tariffa oraria €" size="small" type="number" value={form.laborRate}
              inputProps={{ min:0, step:0.5 }} onChange={e => setForm((f: any) => ({ ...f, laborRate: e.target.value }))} />
            <Box sx={{ display:'flex', alignItems:'center', justifyContent:'flex-end' }}>
              <Typography variant="body2" color="text.secondary" sx={{ mr:1 }}>Manodopera:</Typography>
              <Typography fontWeight={600}>{formatEur(parseFloat(form.laborHours||'0')*parseFloat(form.laborRate||'0'))}</Typography>
            </Box>
          </Box>

          <Box sx={{ display:'flex', alignItems:'center', gap:2, mb:2, bgcolor:'#F6F8FF', borderRadius:2, px:2, py:1.5 }}>
            <FormControl size="small" sx={{ minWidth:200 }}>
              <InputLabel>Esito</InputLabel>
              <Select value={form.outcome} label="Esito" onChange={e => setForm((f: any) => ({ ...f, outcome: e.target.value }))}>
                {OUTCOMES.map(o => (
                  <MenuItem key={o.value} value={o.value}>
                    <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>
                      <Box sx={{ width:10, height:10, borderRadius:'50%', bgcolor:o.color }} />{o.label}
                    </Box>
                  </MenuItem>
                ))}
              </Select>
            ...
            <Box sx={{ textAlign:'right' }}>
              ...
              {total > 77.47 && <Typography variant="caption" color="text.secondary">+ €2,00 bollo</Typography>}
            </Box>
          </Box>
          ...
          {/* Sezione allegati immagini — solo se intervento esistente */}
          {selected && (
            <>
              <Divider sx={{ my:2 }}><Typography variant="caption" color="text.secondary">FOTO ALLEGATE (targhetta, macchina)</Typography></Divider>
              <Box sx={{ mb:1 }}>
                {attachments.length > 0 && (
                  <Stack spacing={0.5} sx={{ mb:1.5 }}>
                    {attachments.map(att => (
                      <Box key={att.id} sx={{
                        display:'flex', alignItems:'center', gap:1.5,
                        bgcolor:'#F6F8FF', borderRadius:2, px:1.5, py:0.8,
                        border:'0.5px solid', borderColor:'divider'
                      }}>
                        <ImageOutlinedIcon fontSize="small" color="action" />
                        <Typography variant="body2" sx={{ flex:1 }} noWrap>{att.filename}</Typography>
                        <Typography variant="caption" color="text.secondary">{formatBytes(att.sizeBytes)}</Typography>
                        <Tooltip title="Apri immagine">
                          <IconButton size="small" onClick={() => window.open(att.driveUrl,'_blank')} sx={{ color:'primary.main' }}>
                            <OpenInNewIcon fontSize="small"/>
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="Elimina">
                          <IconButton size="small" onClick={() => handleDeleteAttachment(att.id)} sx={{ color:'error.main' }}>
                            <DeleteOutlineIcon fontSize="small"/>
                          </IconButton>
                        </Tooltip>
                      </Box>
                ...
                <Button startIcon={uploadingImg ? <CircularProgress size={16}/> : <UploadFileIcon/>}
                  size="small" variant="outlined" disabled={uploadingImg}
                  onClick={() => fileInputRef.current?.click()}>
                  {uploadingImg ? 'Caricamento...' : 'Carica foto'}
                </Button>
                <Typography variant="caption" color="text.secondary" sx={{ ml:1 }}>
                  JPG, PNG, PDF — salvate su Drive, non stampate
                </Typography>
              </Box>
            </>
          )}
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt:1 }}>
            Il PDF e le foto su Drive verranno eliminati.
          </Typography>
        </DialogContent>
        <DialogActions>
